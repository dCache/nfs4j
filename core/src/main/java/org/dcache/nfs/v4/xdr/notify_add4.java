...
public class notify_add4 implements XdrAble {
    ...
    public void xdrEncode(XdrEncodingStream xdr)
            throws OncRpcException, IOException {
        {
            int $size = nad_old_entry.length;
            xdr.xdrEncodeInt($size);
            for (int $idx = 0; $idx < $size; ++$idx) {
                nad_old_entry[$idx].xdrEncode(xdr);
            }
        }
        nad_new_entry.xdrEncode(xdr);
        {
            int $size = nad_new_entry_cookie.length;
            xdr.xdrEncodeInt($size);
            for (int $idx = 0; $idx < $size; ++$idx) {
                nad_new_entry_cookie[$idx].xdrEncode(xdr);
            }
        }
        {
            int $size = nad_prev_entry.length;
            xdr.xdrEncodeInt($size);
            for (int $idx = 0; $idx < $size; ++$idx) {
                nad_prev_entry[$idx].xdrEncode(xdr);
            }
        }
    ...
    public void xdrDecode(XdrDecodingStream xdr)
            throws OncRpcException, IOException {
        {
            ...
            nad_old_entry = new notify_remove4[$size];
            for (int $idx = 0; $idx < $size; ++$idx) {
                nad_old_entry[$idx] = new notify_remove4(xdr);
            }
        }
        nad_new_entry = new notify_entry4(xdr);
        {
            int $size = xdr.xdrDecodeInt();
            nad_new_entry_cookie = new nfs_cookie4[$size];
            for (int $idx = 0; $idx < $size; ++$idx) {
                nad_new_entry_cookie[$idx] = new nfs_cookie4(xdr);
            }
        }
        {
            int $size = xdr.xdrDecodeInt();
            nad_prev_entry = new prev_entry4[$size];
            for (int $idx = 0; $idx < $size; ++$idx) {
                nad_prev_entry[$idx] = new prev_entry4(xdr);
            }
        }
