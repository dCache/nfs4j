...
public class EXCHANGE_ID4args implements XdrAble {
    ...
    public void xdrEncode(XdrEncodingStream xdr)
        ...
        {
            int $size = eia_client_impl_id.length;
            xdr.xdrEncodeInt($size);
            for (int $idx = 0; $idx < $size; ++$idx) {
                eia_client_impl_id[$idx].xdrEncode(xdr);
            }
        }
    }
    ...
    public void xdrDecode(XdrDecodingStream xdr)
        ...
        eia_state_protect = new state_protect4_a(xdr);
        {
            ...
            eia_client_impl_id = new nfs_impl_id4[$size];
            for (int $idx = 0; $idx < $size; ++$idx) {
                eia_client_impl_id[$idx] = new nfs_impl_id4(xdr);
            }
        }
    }
