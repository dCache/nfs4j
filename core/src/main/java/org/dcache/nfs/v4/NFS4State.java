...
public class NFS4State {
     ...
     *
     * This structure is used for the various state sharing mechanisms between the client and server. For the client,
     * this data structure is read-only. The starting value of the seqid field is undefined. The server is required to
     * increment the seqid field monotonically at each transition of the stateid. This is important since the client
     * will inspect the seqid in OPEN stateids to determine the order of OPEN processing done by the server.
     *
     */
    ...
    /**
     * A state (lock, layout)) can be derived from an open state. If null, then this is the original open state.
     */
    ...
    /**
     * Release resources used by this State if not released yet. Any subsequent call will have no effect.
     *
